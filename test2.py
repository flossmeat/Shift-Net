...
import time
import argparse
from basicsr.models.archs.gshift2 import GShiftNet
    ...
    def infer(self):
        with torch.no_grad():
            ...
            for v in videos:
                ...
                end_frames = 2
                one_len = 16
                k_len = (len(input_frames)) // one_len
                for kk in range(k_len):
                    start_time = time.time()
                    in_seq = input_frames[kk*one_len:kk*one_len +one_len + begin_frames+end_frames]
                    gt_seq = gt_frames[kk*one_len+begin_frames:kk*one_len+one_len+begin_frames]
                    ...
                    # print(output.max(), output.min(), output_stage1.max(), output_stage1.min())
                    # output_img = self.tensor2numpy(output)
                    # print(output.shape, len(gts), gts[0].shape)
                    # exit(0)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GSHIFTNet-Inference')
    ...
    parser.add_argument('--data_path', type=str, default='./dataset/GOPRO/test',
                        help='the path of test data')
    parser.add_argument('--model_path', type=str, default='./checkpoints/shiftnet_small.pth',
