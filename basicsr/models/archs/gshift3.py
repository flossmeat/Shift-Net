...
class GShiftNet(nn.Module):
    ...
    def __init__(self, n_features=48, future_frames=1, past_frames=1):
        ...
        self.feat_extract = nn.Sequential(nn.Conv2d(3, self.n_feats0, 3, 1, 1),
               nn.PReLU(), CAB(self.n_feats0, 3, 4, bias=False, act=nn.PReLU()))
        #self.feat_extract2 = CAB(24, 3, 4, bias=False, act=nn.PReLU())
        #self.feat_down = nn.Sequential(nn.Conv2d(24, self.n_feats, 2, 2, 0, bias=False),
        #          nn.PReLU())
        ...
        #         nn.LeakyReLU(negative_slope=0.1, inplace=True))
        # mid_channels = 64
        # self.rorb6 = ResidualBlocksWithInputConv(self.n_feats, mid_channels, 5)
        ...
        # self.conv_hr = nn.Conv2d(64, self.n_feats, 3, 1, 1, bias=False)
        self.conv_last = nn.Conv2d(self.n_feats0, 3, 3, 1, 1, bias=False)
        ...
        self.conv_trans = conv(self.n_feats0, self.n_feats0, 3, bias=False)
        ...
        self.rconcat = nn.Conv2d(self.n_feats0*3, self.n_feats0, 3, 1, 1, bias=False)
    ...
    def stage2(self, x0, sam1_feats, decoder_out0):
        x = self.lrelu(self.rconcat(torch.cat((x0, sam1_feats, decoder_out0), dim=1)))
        # x = checkpoint.checkpoint(self.rorb1, x)
        #x = self.stage2_conv(x)
        #shortcut0 = x
        #x = self.rconcat_down(x)
    ...
    def forward(self, x, k1=None, k2=None, k3=None):
        ...
        # deblurred_img = self.out(output_features, shortcut[self.num_fb:frames-self.num_ff])
