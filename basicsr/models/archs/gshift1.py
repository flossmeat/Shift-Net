...
class CAB1(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        ...
        modules_body.append(conv(n_feat, n_scale_feat, 1, bias=bias))
...
class CAB2(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act, add_channel=0):
        ...
        modules_body.append(conv(n_feat + self.add_channel, n_scale_feat, 1, bias=bias))
        modules_body.append(nn.GELU())
        modules_body.append(RepConv(n_scale_feat, kernel_size, bias))
        modules_body.append(conv(n_scale_feat, 2*n_scale_feat, 1, bias=bias))
        modules_body.append(SimpleGate())
...
class DownSample(nn.Module):
    def __init__(self, in_channels, s_factor):
        ...
        self.down = nn.Sequential(nn.Conv2d(in_channels, in_channels + s_factor, kernel_size=3, stride=2, padding=1, bias=False),
                            nn.PReLU())
    def forward(self, x):
        x = self.down(x)
        return x
...
class Encoder2(nn.Module):
    def __init__(self, n_features, kernel_size=3, reduction=4, bias=False, scale_unetfeats=48):
        ...
        self.concat = CAB(24, kernel_size, reduction, bias=bias, act=act)
        self.down01 = nn.Sequential(nn.Conv2d(24, n_feat, 3, 2, 1, bias=True), nn.PReLU())
        # self.act = act
        ...
        self.upsample0 = PixelShufflePack(n_feat, 24, 2, upsample_kernel=3)
        self.skip_conv = CAB(24, kernel_size, reduction, bias=bias, act=act) #conv(n_feat, n_feat, kernel_size, bias=bias)
        self.out_conv = CAB(24, kernel_size, reduction, bias=bias, act=act)
        self.conv_hr0 = conv(24, 24, kernel_size, bias=bias)
...
class GShiftNet(nn.Module):
    ...
    def __init__(self, n_features=50, future_frames=1, past_frames=1):
        ...
        self.feat_extract = nn.Sequential(nn.Conv2d(3, 24, 3, 1, 1),
               nn.PReLU())
        self.feat_extract2 = nn.Sequential(CAB(24, 3, 4, bias=False, act=nn.PReLU()), nn.Conv2d(24, self.n_feats, 3, 2, 1),
                 nn.PReLU())
        # self.feat_extract = nn.Sequential(nn.Conv2d(3, self.n_feats, 3, 2, 1),
        #        nn.LeakyReLU(negative_slope=0.1, inplace=True))
        # self.feat_extract2 = nn.Sequential(nn.Conv2d(self.n_feats, self.n_feats, 3, 2, 1),
        #         nn.LeakyReLU(negative_slope=0.1, inplace=True))
        mid_channels = 64
        self.reconstruction = ResidualBlocksWithInputConv(self.n_feats, mid_channels, 5)
        # self.reconstruction2 = ResidualBlocksWithInputConv(self.n_feats, mid_channels, 2)
        self.upsample1 = PixelShufflePack(
            mid_channels, mid_channels, 2, upsample_kernel=3)
        # self.upsample2 = PixelShufflePack(
        #     mid_channels, 64, 2, upsample_kernel=3)
        self.conv_lr = nn.Conv2d(64+self.n_feats,64,3,1,1,bias=False)
        self.conv_hr = nn.Conv2d(64, 64, 3, 1, 1, bias=False)
        self.conv_last = nn.Conv2d(64, 3, 3, 1, 1, bias=False)
        #self.upsample1_2 = PixelShufflePack(
        #    mid_channels, mid_channels, 2, upsample_kernel=3)
        # self.upsample2_2 = PixelShufflePack(
        #     mid_channels, 64, 2, upsample_kernel=3)
        # self.conv_lr = nn.Conv2d(64+self.n_feats,64,3,1,1,bias=False)
        #self.conv_hr_2 = nn.Conv2d(64, 64, 3, 1, 1, bias=False)
        #self.conv_last_2 = nn.Conv2d(64, 3, 3, 1, 1, bias=False)
        self.conv_trans = conv(24, 24, 3, bias=False)
        self.lrelu = nn.PReLU() # nn.LeakyReLU(negative_slope=0.1, inplace=True)


        # RNN cell
        # self.shallow_cell = shallow_cell(self.n_feats)
        # self.stage0_encoder = Encoder(self.n_feats)
        # self.stage0_decoder = Decoder(self.n_feats)
        self.upsample0 = PixelShufflePack(
            self.n_feats, 24, 2, upsample_kernel=3)
        self.conv_hr0 = nn.Conv2d(24, 24, 3, 1, 1, bias=False)
        self.stage0_skip = CAB(24, 3, 4, bias=False, act=nn.PReLU()) #conv(self.n_feats, self.n_feats, 3, bias=False)
        self.stage0_conv = CAB(24, 3, 4, bias=False, act=nn.PReLU())
        self.stage1 = Encoder2(self.n_feats2)
        # self.sam01 = SAM(self.n_feats2, kernel_size=1, bias=False)
        self.orb1 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        self.orb2 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        self.orb3 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        # self.orb4 = 
        self.orb4 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        self.orb5 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        # self.orb6 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        # self.recons = conv5x5(self.n_feats, 3)
        self.rorb1 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        self.rorb2 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        self.rorb3 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        # self.orb4 = 
        self.rorb4 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        self.rorb5 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        # self.rorb6 = TFR_UNet(self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False, scale_unetfeats=0)
        # self.rshallow_cell = shallow_cell(self.n_feats)
        self.rconcat = nn.Conv2d(24*3, self.n_feats, 3, 2, 1, bias=True)
        
        div = 4
        self.slice_c =  self.n_feats // div
    def stage0(self, x0):
        # x0 = self.shallow_cell(x)
        # x0 = checkpoint.checkpoint(self.orb1, x0)
        # x0 = checkpoint.checkpoint(self.orb2, x0)
        # x0 = checkpoint.checkpoint(self.orb3, x0)
        # x0 = checkpoint.checkpoint(self.orb4, x0)
        # x0 = checkpoint.checkpoint(self.orb5, x0)
        shortcut0 = x0
        x0 = self.feat_extract2(x0)
        shortcut = x0
        x0 = self.orb1(x0)
        x0 = self.orb2(x0)
        x0 = self.orb3(x0)
        ...
        # res0 = self.orb6(x0)
        res0 = res0 + shortcut
        res0 = self.conv_hr0(self.lrelu(self.upsample0(res0))) + self.stage0_skip(shortcut0)
        res0 = self.stage0_conv(res0)
        # feat0 = self.stage0_encoder(x0)
        # res0 = self.stage0_decoder(feat0)
        # stage0_img = self.upsampling2(res0, shortcut1)
        # sam1_feats, stage0_img = self.sam01(res0[0], x)
        # sam1_feats, stage0_img = 
        return res0, self.lrelu(self.conv_trans(res0)) # , stage0_img
    def stage2(self, x, sam1_feats, decoder_out0):
        x = self.lrelu(self.rconcat(torch.cat((x, sam1_feats, decoder_out0), dim=1)))
        # x = checkpoint.checkpoint(self.rorb1, x)
        # x = checkpoint.checkpoint(self.rorb2, x)
        shortcut = x 
        x = self.rorb1(x)
        x = self.rorb2(x)
        x = self.rorb3(x)
        ...
        x = self.rorb5(x)
        # x = self.rorb6(x)
        # x = checkpoint.checkpoint(self.rorb4, x)
        # x = checkpoint.checkpoint(self.rorb5, x)
        # x = checkpoint.checkpoint(self.rorb6, x)
        # x = self.rorb1(x)
        # x = self.rorb2(x)
        # x = self.rorb3(x)
        # x = self.rorb4(x)
    ...
    def channel_shift(self, x1, div=4):
        B, C, H, W = x1.shape
        slice_c = C // div
        x1_shift = x1[:,-slice_c:,...]
        return x1_shift
    def upsampling(self, lr, shortcut):
        hr = self.reconstruction(lr)
        # hr = self.lrelu(self.conv_lr(torch.cat((hr, shortcut1), dim=1)))
        hr = self.lrelu(self.upsample1(hr))
        # hr = self.lrelu(self.upsample2(hr))
        hr = self.lrelu(self.conv_hr(hr))
        hr = self.conv_last(hr)
        return hr + shortcut
    def upsampling2(self, lr, shortcut):
        hr = self.reconstruction2(lr)
        hr = self.lrelu(self.upsample1_2(hr))
        # hr = self.lrelu(self.upsample2_2(hr))
        hr = self.lrelu(self.conv_hr_2(hr))
        hr = self.conv_last_2(hr)
        return hr + shortcut
    ...
    def forward(self, x, k1=None, k2=None, k3=None):
        ...
        # x1 = self.feat_extract2(x0)
        # print(x.shape)
        # exit(0)

        sam_features0, sam_features = self.stage0(x0) # 10.8, 6.45M
        # return stage0_img
        # return 
        # decoder_outs =  checkpoint.checkpoint(self.stage1, sam_features)
        #print(sam_features.shape)
        decoder_outs = self.stage1(sam_features) # 22.8, 8.7M
        # return
        #print(decoder_outs.shape)
        ...
        # return output_features # 33.9, 15.19M
        # deblurred_img = self.recons(output_features) + x[self.num_fb:frames-self.num_ff]
        deblurred_img = self.upsampling(output_features, shortcut[self.num_fb:frames-self.num_ff])
        # print(stage0_img.shape, deblurred_img.shape)
...
if __name__ == '__main__':
    ...
    net = GShiftNet(future_frames=0, past_frames=0)

    # inp_shape = (num_frames, 3, 256, 256)

    from thop import profile
    FLOPS = 0
    input_ = torch.randn(1, num_frames, 3, 256, 256)
    macs, params = profile(net, inputs=(input_, ))
    # params = float(params[:-4])
    # print(params)
    macs = macs / 10 ** 9 / num_frames + FLOPS / 10 ** 9

    print('macs (G) for Each Frame:', macs, "Params (M):", params)