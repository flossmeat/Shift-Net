...
class CAB(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        ...
        # modules_body.append(nn.Conv2d(n_feat, n_feat*2, kernel_size, bias=bias, padding=kernel_size//2, groups=2))
...
class Encoder2(nn.Module):
    ...
    def forward(self, x, reverse=False):
        ...
        return dec11 # [enc11, enc22, enc33], [dec11, dec22, dec33]
...
class GShiftNet(nn.Module):
    ...
    def __init__(self, n_features=24, future_frames=1, past_frames=1):
        ...
        self.feat_extract = nn.Sequential(nn.Conv2d(3, self.n_feats, 3, 1, 1),
               nn.PReLU(), CAB(self.n_feats, 3, 4, bias=False, act=nn.PReLU()))
        ...
        #         nn.LeakyReLU(negative_slope=0.1, inplace=True))
        mid_channels = 64
        self.reconstruction = ResidualBlocksWithInputConv(self.n_feats, mid_channels, 4)
        ...
        self.conv_trans = conv(self.n_feats, self.n_feats2, 3, bias=False)
        ...
        self.rconcat = conv(self.n_feats*2+self.n_feats2, self.n_feats, kernel_size=3, bias=False)
    ...
    def stage2(self, x, sam1_feats, decoder_out0):
        x = self.rconcat(torch.cat((x, sam1_feats, decoder_out0), dim=1))
    ...
    def forward(self, x, k1=None, k2=None, k3=None):
        ...
        # x2 = self.feat_extract2(x1)
        # print(x.shape)
        # exit(0)

        sam_features0, sam_features = self.stage0(x1) # 10.8, 6.45M
        # return stage0_img
        # return 
        # decoder_outs =  checkpoint.checkpoint(self.stage1, sam_features)
        decoder_outs = self.stage1(sam_features) # 22.8, 8.7M
        # return
        output_features = self.stage2(x1[self.num_fb:frames-self.num_ff], sam_features0[self.num_fb:frames-self.num_ff], decoder_outs[self.num_fb:frames-self.num_ff])
